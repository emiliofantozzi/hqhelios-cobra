...
- [ ] RLS ENABLED en tabla collections
- [ ] RLS FORCED en tabla collections
- [ ] Policies SELECT, INSERT, UPDATE, DELETE creadas
- [ ] Partial unique index creado
- [ ] Queries usando Supabase Client (no Prisma directo)
- [ ] tenant_id extraído de Clerk metadata
- [ ] FKs validadas (invoice, company, contact, playbook existen)
- [ ] Tests de aislamiento RLS ejecutados
- [ ] Test de partial unique index ejecutado
- [ ] Performance de índices verificada
...
- `prisma/migrations/XXXXXX_add_collection_schema/migration.sql` - Generated by Prisma
- `scripts/rls/04-collections-rls.sql` - RLS policies + partial unique index
- `tests/db/collection-constraints.test.ts` - Constraint tests
- `tests/rls/collection-isolation.test.ts` - RLS isolation tests
- `tests/performance/collection-worker.test.ts` - Performance tests
